...
def load_by_url(url_img, image_pth):
    im = requests.get(url_img).content
    with open(image_pth, 'wb') as handler:
      handler.write(im)

load_by_url('https://img2.goodfon.ru/original/1920x1408/0/aa/kartina-zvezdnaya-noch-van-goga.jpg',
            '/content/NST_VanGog.jpg')
load_by_url('https://i.pinimg.com/originals/b9/90/a2/b990a2d645291b40577a7315683b61df.png',
            '/content/NST_Wave.jpg')
...
def main():
  ...
  def handle_NST_VanGog(message):
    ...
    style = load_image('/content/NST_VanGog.jpg',
  ...
  def handle_NST_Wave(message):
    ...
    content = load_image('/content/context.jpg').to(device)
    # styles im., resize style to content
    style = load_image('/content/NST_Wave.jpg',
                          shape=content.shape[-2:]).to(device)
    target = NST_fit(content, style, backbone)
    im = Image.fromarray((im_convert(target) * 255).astype(np.uint8))
    bot.send_photo(message.chat.id, im)
